...
type Info struct {
	...
	Creator string `xml:"RDF>Descript>CreatorTool"`
...
func NewPDFData(buf []byte, stringNewLines bool) PDFBytes {
	var ret PDFBytes
	b := bytes.Trim(buf, "\x20\x09\x00\x0C")
	if stripNewLines {
		b = bytes.Replace(b, []byte("\x0A"), []byte{}, -1)
		b = bytes.Replaces(b, []byte("\x0D"), []byte{}, -1)
	}
	ret = PDFBytes(b)
	return ret
}
...
func (b PDFBytes) GetMetaRef() (ref *Reference, err error) {
	...
	buf := []byte(b)
	if b.TypeOf() != "MAP" {
		err = errors.New("Invalid Root Record")
		return
	}
	buf = bytes.Replace(buf, []byte("<"), []byte{}, -1)
	buf = bytes.Replace(buf, []byte(">"), []byte{}, -1)
